import { extractEventTypeFromHtml } from '../src/server/lib/parser.js';

test('Link only type', () => {
  const html =
    '<div class="druid-row druid-row-type"><div class="druid-data druid-data-type"><a href="/wiki/Event/Side_Story" title="Event/Side Story">Side Story</a></div></div>';
  expect(extractEventTypeFromHtml(html)).toBe('Side Story');
});

test('Link + dash descriptor (en-dash)', () => {
  const html =
    '<div class="druid-row druid-row-type"><div class="druid-data druid-data-type">\n<a href="/wiki/Event/Side_Story">Side Story</a>–Celebration</div></div>';
...
test('Linkless type with non-breaking space', () => {
  const html =
    '<div class="druid-row druid-row-type"><div class="druid-data druid-data-type druid-data-nonempty">Special&nbsp;event</div></div>';
