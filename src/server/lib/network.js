...
const { parseIndexHtml } = require('./parser');
  ...
  return new Promise((resolve, reject) => {
    ...
    https.get(apiUrl, options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (typeof data === 'string' && data.trim().startsWith('<')) return resolve({ statusCode: res.statusCode, body: null, blocked: true });
        try {
...
function downloadImage(url, filepath) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (url && url.startsWith('/')) url = new URL(url, config.wikiBase).toString();
    } catch (e) {}
    const options = {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
  ...
  try {
    ...
    if (api) {
      if (api.blocked) return null; // blocked by cloudflare or similar
        if (api.statusCode === 200 && api.body && api.body.parse && api.body.parse.text && api.body.parse.text['*']) {
...
module.exports = { fetchWikiApi, downloadImage, fetchEventDetailsViaApi, fetchEventsViaApi, fetchUpcomingViaApi };
