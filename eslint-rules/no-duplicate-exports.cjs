// Custom ESLint rule (CommonJS): no-duplicate-exports
// Flags when the same exported identifier is exported more than once in a file.
...
module.exports = {
  ...
  create(context) {
    // Track exported names and their node locations
    ...
    return {
      ...
      ExportDefaultDeclaration(node) {
        // If default export is an object literal, capture its properties
        const decl = node.declaration;
      ...
      'Program:exit'() {
        // Report any names present in both namedExports and defaultProps
