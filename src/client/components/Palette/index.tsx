...
type SwatchMode = 'fill' | 'border' | 'text' | 'text-on-limited';
...
function swatchStyle(token: string, mode: SwatchMode): CSSProperties {
  switch (mode) {
    case 'text':
...
function ColorSwatch({ token, name, mode }: Swatch) {
...
export function Palette({ label }: PaletteProps) {
  return (
